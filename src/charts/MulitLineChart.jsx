...
export const MultiLineChart=({
    data=[],//expect array of of data. each array represents each line
    width=1000,
    height=400,
  ...
  <span className="visually-hidden">Loading...</span>
    ...
    .domain(d3.extent(flatData,xValue))
    .range([0,innerWidth]);

    const yScale=d3.scaleLinear()
    .domain(d3.extent(flatData,yValue))
    .range([innerHeight,0]);
    ...
    if (trackMouse){
        ...
        handleMouseMove=e=>{
            ...
            let least=d3.least(flatData,d=>Math.hypot(((xScale(xValue(d))-x))));
...
<svg width={width} height={height}  onMouseMove={handleMouseMove}>
                ...
                xValue={xValue}
                yValue={yValue}
                color={displayDiff?(yValue(d[0])>yValue(d[d.length-1])?'#FF0000':'#008000'):lineColor}
                lineWidth={lineWidth}
            />);
                })}
