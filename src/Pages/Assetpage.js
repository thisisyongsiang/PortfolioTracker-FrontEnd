...
import { getOneUserPortfolioValue ,getUserPortfolioAssetsTransactions,getUserPortfolioHistoricalValue, getUserPortfolioOneAssetHistoricalValue, getUserPortfolioOneAssetValue} from "../users/userApi.js";
...
export const Assetpage=(user)=>{
  ...
  useEffect(()=>{
    ...
    if (userData && assetId &&portfolioId){
      ...
      getUserPortfolioOneAssetHistoricalValue(userData.emailAddress,
        portfolioId,assetId,
        new Date(new Date().setFullYear(new Date().getFullYear()-1)),
        new Date()).then(d=>{
          setAssetValueHistory(d);
        });
      getUserPortfolioAssetsTransactions(userData.emailAddress,
        portfolioId,assetId).then(d=>{
          setTransactions(d);
        });
      getUserPortfolioOneAssetValue(
        userData.emailAddress,portfolioId,assetId)
        .then(d=>{
  ...
  <React.Fragment>
    <Container id="container">
      ...
      <div className="row">
        ...
        <hr />
        <h4>Current Price: ${assetQuote?assetQuote["regularMarketPrice"].toFixed(2):"0.00"}</h4>
